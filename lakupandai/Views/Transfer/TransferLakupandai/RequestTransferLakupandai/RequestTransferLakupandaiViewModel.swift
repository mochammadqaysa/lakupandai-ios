//
//  RequestTransferLakupandaiViewModel.swift
//  lakupandai
//
...
class RequestTransferLakupandaiViewModel : ObservableObject {
    @Published var noHPNasabah = ""
    @Published var noHPTujuan = ""
    ...
    func validateForm() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
            if self.nominal.isEmpty || self.noHPNasabah.isEmpty || self.noHPTujuan.isEmpty {
                self.showAlert = true
            ...
            } else if self.noHPTujuan.count < 10 {
                self.showAlert = true
                self.alertMessage = ConstantError.ALERT_NUMBER_LESS_THAN_10
                ...
                self.requestTransferLakupandai()
            }
            self.isLoading = false
        }
    }
    
    func requestTransferLakupandai() {
        ...
        let params: [String: Any] = [
            ...
            ConstantTransaction.MSISDN_TUJUAN_REQUEST_TRANSFER : self.noHPTujuan,
            ConstantTransaction.ACTION : ConstantTransaction.ACTION_REQUEST_TRANSFER_LAKUPANDAI
        ]
        do {
