//
//  KonfirmasiRequestTransferBankViewModel.swift
//  lakupandai
//
...
class KonfirmasiRequestTransferBankViewModel : ObservableObject {
    ...
    func konfirmasiRequestTransferLakupandai(token : String) {
        ...
        let params: [String: Any] = [
            ...
            ConstantTransaction.ACTION : ConstantTransaction.ACTION_REQUEST_TRANSFER_KONFIRMASI_ANTAR_BANK
        ]
...
//                                self.dataToken = rmToken
                            ...
                            if let rmString = resjson!["RM"] as? String, let rmData = try JSONSerialization.jsonObject(with: rmString.data(using: .utf8)!, options: []) as? [String: Any] {
                                if let data = rmData["data"] as? [[String: String]] {
                                    ...
                                    UserDefaultsManager.shared.saveString(resultEnc!, forKey: "temp_body_transfer_bank")
                                    for rmDatum in data {
