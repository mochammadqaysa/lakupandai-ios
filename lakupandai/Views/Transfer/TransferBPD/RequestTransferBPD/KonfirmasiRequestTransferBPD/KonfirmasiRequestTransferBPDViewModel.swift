//
//  KonfirmasiRequestTransferBPDViewModel.swift
//  lakupandai
//
...
class KonfirmasiRequestTransferBPDViewModel : ObservableObject {
    ...
    func validateForm(token : String) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
            if self.pinAgen.isEmpty {
                ...
                self.konfirmasiRequestTransferBPD(token: token)
            }
    ...
    func konfirmasiRequestTransferBPD(token : String) {
        ...
        let params: [String: Any] = [
            ...
            ConstantTransaction.ACTION : ConstantTransaction.ACTION_REQUEST_TRANSFER_KONFIRMASI
        ]
...
//                                self.dataToken = rmToken
                            ...
                            if let rmString = resjson!["RM"] as? String, let rmData = try JSONSerialization.jsonObject(with: rmString.data(using: .utf8)!, options: []) as? [String: Any] {
                                if let data = rmData["data"] as? [[String: String]] {
                                    ...
                                    UserDefaultsManager.shared.saveString(resultEnc!, forKey: "temp_body_transfer_bpd")
                                    for rmDatum in data {
