//
//  KonfirmasiTransferBPDDetailViewModel.swift
//  lakupandai
//
...
class KonfirmasiTransferBPDDetailViewModel : ObservableObject {
    ...
    func validateForm(token : String) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
            if self.pinAgen.isEmpty {
                ...
                self.konfirmasiTransferBPD(token: token)
            }
    ...
    func konfirmasiTransferBPD(token : String) {
        ...
        let params: [String: Any] = [
            ...
            ConstantTransaction.ACTION : ConstantTransaction.ACTION_TRANSFER_KONFIRMASI_DETAIL
        ]
        do {
...
//
                                    ...
                                    if let resi = dataResi, let waktu = dataWaktu, let rekPenerima = dataRekPenerima, let namaPenerima = dataNamaPenerima, let nominal = dataNominal, let namaPengirim = dataNamaPengirim, let noHpPengirim = dataNoHPPengirim, let status = dataStatus {
                                    ...
                                    UserDefaultsManager.shared.deleteKey(forKey: "temp_body_transfer_bpd")
                                    self.nextStep = true
